...
def generate_blog(request):
    if request.method == 'POST':
        ...
        for name in os.listdir(folder_path):
            audio_path = os.path.join(folder_path, name)
...
def generate_blog_from_transcription(transcription):
    ...
    blog_content = response.text[:20000]
    remove_chars = string.punctuation.replace('.', '') + "\n*"
    table = str.maketrans('', '', remove_chars)
    clean_content = blog_content.translate(table)
    escaped_content = clean_content.replace('"', '\\"')
    json_response = json.dumps(escaped_content)
...
def blog_list(request):
    ...
    latest_blog_post = BlogPost.objects.latest('created_at')
    context = {
...
def blog_details(request, pk):
    ...
    if request.user == blog_article_detail.user:
        print(f"Created at: {blog_article_detail.created_at}")
